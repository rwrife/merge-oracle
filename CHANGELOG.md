...
### Added
- `oracle duel` command: comparative reading between two contender diffs/PRs. Loads both sides via existing source loaders (URL, path, or bare PR number with `-R`; stdin refused because it's ambiguous), draws symbols and per-side readings with the same method + persona, then asks the LLM once more in comparative framing for a structured verdict JSON (`favor-a` | `favor-b` | `favor-neither`, `confidence`, `rationale`, `carryForward`). Renders as `⚔️ contenders → 🃏 reading A → 🃏 reading B → ⚖️ judgement → 🏆 verdict → 🕯️ carry-forward`, or a full `--json` blob under `duel.{a,b,verdict,confidence,rationale,carryForward,judgement}`. `--offline` produces a deterministic verdict synthesized from diff stats (safe for CI snapshots); `--fail-on=<verdict>` exits non-zero for CI bake-offs; identical diffs are refused with a wry error. (#41)
